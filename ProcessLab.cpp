...
uint64 ProcessLab( const uint8* src )
{
    ...
    for( size_t i=0; i<16; i++ )
    {
        ...
        Color::Lab pix( v3b( r, g, b ) );

        for( int t=0; t<8; t++ )
        {
            const int32* tab = g_table[t];
            uint idx = 0;
            Color::Lab test( v3b( clampu8( a[bid].x + tab[0] ), clampu8( a[bid].y + tab[0] ), clampu8( a[bid].z + tab[0] ) ) );
            float err = sq( pix.L - test.L ) + sq( pix.a - test.a ) + sq( pix.b - test.b );
            for( int j=1; j<4; j++ )
            {
                Color::Lab test( v3b( clampu8( a[bid].x + tab[j] ), clampu8( a[bid].y + tab[j] ), clampu8( a[bid].z + tab[j] ) ) );
                float local = sq( pix.L - test.L ) + sq( pix.a - test.a ) + sq( pix.b - test.b );
                if( local < err )
                {
                    err = local;
                    idx = j;
